...
try:
    ...
    def start():
        global running
        running = (running + 1) % 2
        sleep = hrs * 3600 + mins * 60 + secs + ms / 1000
        if sleep < 0.016:
            sleep = 0.016
        try:
    ...
    def stop():
        global running
        if keyboard.is_pressed(endsequence.text()):
            running = 0
    ...
    def hotkys(run):
        ...
        if run:
            ...
            keyboard.on_press_key(sequence.text(), start_wrapper)
            time.sleep(0.1)
            keyboard.on_press_key(endsequence.text(), end_wrapper)
    ...
    import sys, time, threading, pyautogui, keyboard, random, traceback
    ...
    sequence.textEdited.connect(lambda: hotkys(True) if sequence.text() != "" and endsequence.text() != "" else hotkys(False))
    endsequence.textEdited.connect(lambda: hotkys(True) if sequence.text() != "" and endsequence.text() != "" else hotkys(False))
    button = Button.left
